  ...
  if (product && product.quantity > 0) {
    ...
    if (article) {
      if (checkStock(id, qty + article.qty)) {
        ...
        window.alert("Los NFT´S son piezas únicas, si deseas adquirir más, escríbenos un correo para que diseñemos uno especial para ti");
      }
    } else {
      cart.push({ id, qty });
    }
...
function removeFromCart(id, qty = 1) {
  ...
  if (article && article.qty - qty > 0) {
    ...
    const confirm = window.confirm("ESTE NFT ES PIEZA ÚNICA SI TE ARREPIENTES EN UN SEGUNDO ALGUIEN MÁS LO PUEDE TOMAR, ¿Estás Seguro?");
    if (confirm) {
...
function totalAmount() {
  return cart.reduce((acc, article) => {
    ...
    producto por la cantidad de artículos del carrito*/
    const product = products.find((p) => p.id === article.id);
